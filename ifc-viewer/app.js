...
class IFCViewer {
  ...
  handleMouseMove(event) {
    ...
    if (
      this.connectionVisualizer &&
      this.connectionVisualizer.isConnectionMode
    ) {
      ...
      if (result && result.object.userData.hasConnections) {
        ...
        const connectionIds = this.elementConnections.get(elementId);

        if (connectionIds) {
          // Show all connections for this element
          connectionIds.forEach((connectionId) => {
            const modelConnections = Array.from(this.connections.values())[0];
            const connection = modelConnections.get(connectionId);
            if (connection) {
              const visualization =
                this.connectionVisualizations.get(connectionId);
              this.connectionVisualizer.handleHover({
        ...
        this.connectionVisualizer.handleHover(null);
  ...
  setupFloatingMenu() {
    ...
    if (modelsContent) {
      ...
      typeToggles.forEach((toggle) => {
        toggle.addEventListener("change", (e) => {
          ...
          if (this.connectionVisualizer) {
            this.connectionVisualizer.setTypeVisibility(type, e.target.checked);
          }
        });
      });
      ...
      labelToggle.addEventListener("change", (e) => {
        if (this.connectionVisualizer) {
          this.connectionVisualizer.setGlobalLabelVisibility(e.target.checked);
        }
      });
    }
  }
  ...
  async analyzeConnections() {
    ...
    try {
      ...
      // Map to store elements and their connections
  ...
  exitConnectionMode() {
    ...
    if (this.connectionVisualizer) {
      ...
      this.connectionVisualizations.forEach((vis) => {
        ...
        if (vis.surface) this.scene.remove(vis.surface);
      });
  ...
  visualizeConnections(modelId, connections) {
    ...
    // Don't call updateLabels here as it will show all labels
  }
    ...
    Object.entries(connectionsByType).forEach(([type, typeConnections]) => {
      ...
      typeConnections.forEach((connection) => {
        ...
        item.addEventListener("click", () => {
          ...
          const visualization = this.connectionVisualizations.get(
            connection.id
          );
          if (visualization) {
