...
export class SpatialTree {
  ...
  private async processProject(modelID: number, projectID: number, treeContent: HTMLElement): Promise<void> {
    const project = await this.viewer.ifcAPI.GetLine(modelID, projectID, true) as IFCElement;
  ...
  private async processSites(modelID: number, projectNode: HTMLElement): Promise<void> {
    const siteLines = await this.viewer.ifcAPI.GetLineIDsWithType(modelID, IFCSITE);
    
    for (let j = 0; j < siteLines.size(); j++) {
      const siteID = siteLines.get(j);
      const site = await this.viewer.ifcAPI.GetLine(modelID, siteID, true) as IFCElement;
  ...
  private async processBuildings(modelID: number, siteNode: HTMLElement): Promise<void> {
    const buildingLines = await this.viewer.ifcAPI.GetLineIDsWithType(modelID, IFCBUILDING);
    
    for (let k = 0; k < buildingLines.size(); k++) {
      const buildingID = buildingLines.get(k);
      const building = await this.viewer.ifcAPI.GetLine(modelID, buildingID, true) as IFCElement;
  ...
  private async processStoreys(modelID: number, buildingNode: HTMLElement): Promise<void> {
    const storeyLines = await this.viewer.ifcAPI.GetLineIDsWithType(modelID, IFCBUILDINGSTOREY);
    const storeys: StoreyInfo[] = [];
    
    for (let l = 0; l < storeyLines.size(); l++) {
      const storeyID = storeyLines.get(l);
      const storey = await this.viewer.ifcAPI.GetLine(modelID, storeyID, true) as IFCElement;
  ...
  private setupTreeItemHandlers(treeContent: HTMLElement): void {
    ...
    treeItems.forEach((item) => {
      item.addEventListener("click", (event) => {
        ...
        // Toggle expansion
        if (event.target instanceof Element && event.target.closest(".tree-item-toggle")) {
          ...
          const prevSelected = treeContent.querySelector(".tree-item-header.selected");
          if (prevSelected) {
  ...
  private findElementInScene(modelID: number, expressID: number): THREE.Object3D | null {
